...
TinyPPI.metadata = (function () {
  ...
  const el = {
    ...
    idleTitle: $("metaIdleTitle"), idleText: $("metaIdleText"),
  ...
  function render(next) {
    ...
    /* What is playing and the L1 luminance chart come from the shared live
       module, which the dashboard feeds.  The L5 active-picture card is this
       tab's own. */
    ...
    if (!rows.length) {
      ...
      /* Two different nothings: no film at all, or a film with no RPU behind it.
         Saying which is the difference between a tab that looks broken and one
         that has answered. */
      el.idleTitle.textContent = next.playing ? T.no_metadata : T.idle_title;
      el.idleText.textContent  = next.playing ? T.no_metadata_text : T.idle_text;
      signature = "";
      live = [];
  ...
  /* Whether there is a list to copy: nothing to list is nothing to copy, so the
     button goes with the list rather than staying behind to answer a press with
     nothing. */
  function listed() {
    return !!(state && (state.metadata || []).length);
  }
  ...
  function strings(T) {
    ...
    $("metadataTitle").textContent = T.metadata_section;
    /* The idle card is written by render(); one drawn before the words arrived
       is written again with them. */
    if (state) render(state);
  }

  return { render, strings, copy, listed };
